...
The PPO objective $\mathcal{L}^{\text{PPO}} = \mathcal{L}^{\text{Actor}} + \mathcal{L}^{\text{Critic}} - \beta\mathcal{H}$ optimizes policy $\pi\_{\theta}$ through: (1) $\mathcal{L}^{\text{Actor}}$ using clipped ratio $r\_t(\theta) = \pi\_{\theta}/\pi\_{\theta\_{\text{old}}}$ and advantage $A\_t$, (2) $\mathcal{L}^{\text{Critic}}$ minimizing TD error $(V\_{\theta}(s\_t) - R\_t)^2$, and (3) entropy $\mathcal{H}$ for exploration. Hyperparameters $\epsilon$ clips $r\_t(\theta)$ in $[1-\epsilon,1+\epsilon]$, while $\beta$ weights $\mathcal{H}$. The expectations $\mathbb{E}\_t$ are over trajectories.
